...
def customer_record(fh, n):
    ...
    file = fh.readlines()
    line = fh.readline()
    if( n < len(file) and n >= 0):
        element = file[n].strip().split(",")
        array = element
    else:
        array = []
...
def customer_first(fh):
    ...
    #list_of_files = fh.readlines()
    list_of_files = fh.readline()

    while(list_of_files != ""):
            line = list_of_files.strip().split(",")
            if(line[4] == "1998-01-03"):
                return line
            list_of_files = fh.readline()
...
def find_longest(fh):
    ...
    word = "a"
    while(line != ""):
        if(len(line.strip()) > len(word) or len(line.strip()) == len(word)):
            word = line.strip()
        line = fh.readline()
    return word
