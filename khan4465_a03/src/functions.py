...
def stack_reverse(source):
    ...
    while(source.is_empty() == False):
        new_stack.append(source.pop())
    
    for x in range(len(new_stack)):
        source.push(new_stack[x])
    return None
...
def is_palindrome_stack(string):
    ...
    for x in range(len(string)):
        if(string[x].isalpha() == True):
            new_string+= string[x].lower()
    for x in range(len(new_string)):
        s1.push(new_string[x])
    for x in range(len(new_string)):
        if(new_string[x] == s1.pop()):
            i+=1
    if(i == len(new_string)):
        con = True
    return con
...
def postfix(string):
    ...
    for x in range(len(string)):
        if(string[x].isdigit() == True):
            s1.push(int(string[x]))
            
        elif(string[x] == "-"):
            result = ((s1.pop() - s1.pop())*-1)
            s1.push(result)
        elif(string[x] == "/"):
            num = []
            num.append(s1.pop())
...
def reroute(opstring, values_in):
    ...
    -------------------------------------------------------
    """
    values_in_copy = values_in.copy()
    s1 = Stack()
    i = 0
    values_out = []
    for x in range(len(opstring)):
        if(opstring[x] == "S"):
            s1.push(values_in_copy[i])
            i+=1
        elif(opstring[x] == "X" and s1.is_empty() == False):
