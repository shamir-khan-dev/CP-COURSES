...
public class ExceptionsTest {
	...
	while (true) {
	    ...
	    // If the next token is "Quit", stop the loop
	    if (keyboard.hasNext("Quit")) {
		keyboard.next(); // consume Quit
		break;
	    }

	    try {
		// MUST use nextInt() so InputMismatchException is thrown
		int value = keyboard.nextInt();
		total += value;

	    } catch (InputMismatchException e) {
		System.out.println("That is not an integer!");
		keyboard.next(); // consume the bad token
	    }
	}
