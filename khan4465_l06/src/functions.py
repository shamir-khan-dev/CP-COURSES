...
def gic(value, years, rate):
    ...
    for year in range(years+1):
        # Print year and value formatted with two decimal places and commas
        print(f"{year:>2}{final:>16,.2f}")
        # Update final for the next year
        final = float((final * (rate / 100)) + final)
    
    # Format the final value with commas and two decimal places
...
def statistics(n):
    ...
    for num in range(n-1):
        ...
        if(value > biggest):
            biggest = value
        total+= value
    return smallest,biggest,total,total/n
...
def treadmill(burnt_per_minute, start, end, inc):
    ...
    if(end % 2 == 0 or end == 1): 
        for p in range(start,end+inc,inc):
            calo = float(burnt_per_minute * p)
            print(f"Calories burned after {p} minutes: {calo:.1f}")
        
    else:
        for p in range(start,end,inc):
            calo = float(burnt_per_minute * p)
            print(f"Calories burned after {p} minutes: {calo:.1f}")
    return None
