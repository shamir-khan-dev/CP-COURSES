...
def generate_matrix_num(rows, cols, low, high, value_type):
    ...
    for x in range(rows):
        ...
        for y in range(cols):
            if(value_type == "float"):
                second_list.append(random.uniform(low,high))
            
            else:
                second_list.append(random.randint(low,high))
        
        one_list.append(second_list)
...
def print_matrix_num(matrix, value_type):
    ...
    for a in range(1, len(matrix[0])):  # Correct length for column headings
        print(f"{a:6d}", end="")
    print()
       
    for x in range(len(matrix)):
        print(f" {x}", end=" ")  # Correct alignment for row heading
        
        for y in range(len(matrix[x])):
            if value_type == "float":
                print(f"{matrix[x][y]:6.2f}", end="")
            elif value_type == "int":
                print(f"{matrix[x][y]:6d}", end="")
        print()
    return None
...
def matrix_stats(matrix):
    ...
    for x in range(len(list)):
        ...
        if((list[x]) > largest):
            largest = list[x]
        total+=list[x]
    return small,largest,total,total/len(list)
...
def find_less(matrix, n):
    ...
    for x in range(len(matrix)):
        for y in range(len(matrix[x])):
            if(matrix[x][y] == n and (matrix[x][y+1]) < n):
                num.append(x)
                num.append(y+1)
                break
                found = True
