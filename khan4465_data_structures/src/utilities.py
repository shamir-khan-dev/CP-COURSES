...
def array_to_stack(stack, source):
    ...
    for x in range(len(source)):
        stack.push(source.pop())  # Pop from source and push onto stack
...
def stack_to_array(stack, target):
    ...
    for x in range(1,len(target)+1):
       new.append(target[len(target)-x])
...
def list_to_array(llist, target):
    ...
    while(llist.is_empty() == False):
        new.append(llist.pop())
        
    for x in range(len(new)):
        target.append(new.pop())
...
def list_test(source):
    ...
    print(lst.__contains__(source[0]))
    print(lst.append(source[0]))
    print(lst.count(source[0]))
    print(lst.find(source[0]))
    print(lst.index(source[0]))
